...
*/
const game = "DEMONSLAYERS";//"<---game title"
...
const demon = {
    health:120,
    superAttacks: {
        smackAttack: function(){return Math.floor(Math.random() * 60)},
        knockoutattack: function() {return Math.floor (Math.random() * 61+ 29 )}
        
    }
} 

function randomDemonAttacks(object){
    const superAttacksKeys = Object.keys(object.superAttacks);
    const randomAttack= superAttacksKeys[Math.floor(superAttacksKeys.length*Math.random())]
    const damage = object.superAttacks[randomAttack]()
    console.log("Demon used ",randomAttack," for ",damage," damage ")
    demonSlayer.health -= damage
    console.log(demonSlayer.health,"<---Demon Slayer health")
}

    

function demonSlayerAttacks(object){
    const attacks = Object.keys(object.attacks)
    const damage = object.attacks["punchAttack"]
    console.log("Demon Slayer used ","punchAttack","for",damage,"damage")
    demon.health -= 40 
    console.log(demon.health,"<---Demon Health")
  
} 
    

function runGame (demon,demonSlayer){
    ...
    let gamemessage = "";
    while(demonSlayer.health > 0 && demon.health > 0 ){ 

    
        randomDemonAttacks(demon)
        if (demonSlayer.health >= 0){
            demonSlayerAttacks(demonSlayer)
        }
    }
    gamemessage =  demonSlayer.health <= 0 ? "DemonSlayer Died Demon WINS":"Demon died DemonSlayer WINS"
    // demon.health<= 0 ? "Demon died " :"DemonSlayer Attack"
    return gamemessage 

}
   console.log(runGame(demon,demonSlayer))

    //if demon slayer health is less than or equal top zero demon slayer . die otherwise 
    //demonslay.attack
    
            //return "Demon Slayer You Died"
        
    
//console.log(randomDemonAttacks(demon));
